import{s as k,d as y,e as h,f as p,h as d,i as r,j as C,k as L,l as w,m as q,n as D}from"./assets/workWithCart-597f68ef.js";import{P as B}from"./assets/vendor-b592f4c5.js";const M="/Second_Team-project-2023/assets/discount-1f26b8b0.svg",A="/Second_Team-project-2023/assets/check-ico-267a6277.svg",T=document.querySelector(".popular-product-list");async function _(t){T.insertAdjacentHTML("beforeend",x(t));const e=document.querySelector(".click-buy");e.addEventListener("click",()=>{e.style.display="none",a.style.display="block"});const a=document.querySelector(".popular-check")}function x(t){return t.map(({_id:e,name:a,img:n,category:s,size:c,popularity:i})=>`
      ...
      `).join("")}const v="/Second_Team-project-2023/assets/shopping-cart-a18eb96b.svg",F=document.querySelector(".discount-products-item");function z(t){F.insertAdjacentHTML("beforeend",H(t))}function H(t){return t.map(({_id:e,name:a,img:n,price:s})=>`
    ...
    `,e.modal.classList.add("is-hidden")}}function U(t){C(t.id).then(({data:e})=>{const a=document.querySelector(".product-modal-content");a.innerHTML=V(e)}).catch(e=>console.log(e))}function V(t){return`
  ...
  `}const E=document.querySelector(".product-list");function l(t){E.innerHTML=W(t.results),document.querySelectorAll(".product-modal-open").forEach(a=>{a.addEventListener("click",K(a))})}function W(t){return t.map(({_id:e,name:a,img:n,category:s,price:c,size:i,is10PercentOff:m,popularity:j})=>`
  ...
  <p>Try adjusting your search parameters or browse our range by other criteria to find the perfect product for you. </p>`;E.innerHTML=t,document.querySelector(".js-link").addEventListener("click",X)}function S(t,e){const a=document.querySelector(".modal-text");a.textContent=`Error ${t}: ${e}`,Y()}function Y(){document.querySelector(".backdrop").classList.toggle("is-hidden")}function tt(){const t=document.querySelector(".subscription-form");t.addEventListener("submit",async e=>{e.preventDefault();const n=new FormData(t).get("email");try{const s=`https://food-boutique.b.goit.study/api/subscription?email=${n}`,c=await fetch(s,{method:"GET",headers:{"Content-Type":"application/json"}});if(c.ok){const i=await c.json();console.log("Data retrieved successfully:",i),et(i)}else{const i=await c.json();S(c.status,i.message)}}catch(s){console.error("Error occurred:",s),S(500,"Server error")}})}function et(t){const e=document.querySelector(".subscription-info");e&&(e.textContent=t.message)}tt();const at=document.getElementById("tui-pagination-container");new B(at,{});L().then(({data:t})=>{z(t)}).catch(t=>console.log(t));h().then(({data:t,data:{perPage:e,totalPages:a}})=>{l(t);const n=document.getElementById("tui-pagination-container");new B(n,{totalItems:e*a,itemsPerPage:e,visiblePages:5,centerAlign:!0,template:{page:'<a href="#" class="tui-page-btn">{{page}}</a>',currentPage:'<strong class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}}).on("beforeMove",async c=>{const i=c.page;try{const{data:m}=await d({page:i,limitPerPage:e});l(m)}catch(m){console.log(m)}})});w().then(({data:t})=>{_(t)}).catch(t=>console.log(t));q().then(({data:t})=>{N(t.toString().replaceAll("_"," ").split(","))}).catch(t=>console.log(t));document.getElementById("loader-container");const P=document.querySelector(".main"),b=document.body;window.addEventListener("load",function(){const t=document.querySelector(".loader-container"),e=[L,h,w,q];P.classList.add("visually-hidden"),b.classList.add("no-scroll"),t.classList.remove("visually-hidden"),Promise.all(e.map(a=>a())).then(()=>{P.classList.remove("visually-hidden"),t.classList.add("visually-hidden"),b.classList.remove("no-scroll")}).catch(a=>{console.error("Error in asynchronous operation:",a),t.classList.add("visually-hidden"),b.classList.remove("no-scroll")})});const ot=document.querySelector(".js-main-container");ot.addEventListener("click",t=>{if(t.target.classList.contains("js-buy-btn")){let e=t.target.closest(".js-product-card");D(e.dataset.id),st(e.dataset.id)}});function st(t){const e=document.querySelectorAll(`.js-product-card[data-id="${t}"]`),a="";console.log(e),e.forEach(n=>{const s=n.querySelector(".js-buy-btn");s.innerHTML=`<svg class="img-icon" width="12" height="12" style="stroke:"#fff""><use href="${k}#${a}"></use></svg>`,s.disabled=!0})}
