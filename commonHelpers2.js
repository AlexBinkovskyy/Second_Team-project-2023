import{s as M,d as y,e as m,f as d,h as r,i,j as A,k as C,l as q,m as E,n as T,g as P}from"./assets/workWithCart-5e9a4738.js";import{P as S}from"./assets/vendor-8aa786c2.js";const w="/Second_Team-project-2023/assets/discount-1f26b8b0.svg",b="/Second_Team-project-2023/assets/check-ico-267a6277.svg",_=document.querySelector(".popular-product-list");async function x(t){_.insertAdjacentHTML("beforeend",D(t));const e=document.querySelector(".click-buy");e.addEventListener("click",()=>{e.style.display="none",a.style.display="block"});const a=document.querySelector(".popular-check")}function D(t){return t.map(({_id:e,name:a,img:s,category:n,size:p,popularity:g})=>`
    ...
    `).splice(0,2).join("")}const c=document.querySelector("#filterForm");c.addEventListener("submit",Z);const H=document.querySelector("#single");c.elements.filterCategories.addEventListener("change",U);c.elements.filterMethod.addEventListener("change",G);let o;async function v(){i()&&(o=i(),o.keyword!==null?(c.filterInput.value=o.keyword,await r(i()).then(t=>{if(console.log(t),t.data.results.length)l(t.data);else if(!Array.isArray(t.data.results)||!t.data.results.length){u();return}})):y())}v();function N(t){if(H.insertAdjacentHTML("afterbegin",O(t)),o.category!==null){console.log("1",c.filterCategories);const e=c.filterCategories.options;for(const a of e)if(a.value===o.category){a.value.option.selected;break}}}function O(t){const e=t.map(a=>`
    <option value="${a}" class="js-option">${a}</option>
    `);return e.unshift('<option value="" disabled selected>Categories</option>'),e.join("")}function Z(t){t.preventDefault();const{filterInput:e,filterCategories:a}=t.target.elements;Q(e,a)}function Q(t,e){t.value.trim().length?t.value.trim()?(o.keyword=t.value.trim(),e.value!==""&&(o.category=e.value),d(o),r(i()).then(a=>{if(console.log(a),a.data.results.length)l(a.data);else if(!Array.isArray(a.data.results)||!a.data.results.length){u();return}}).catch(a=>console.log(a))):e.value&&(o.category=e.value.trim(),d(o),r(i()).then(a=>{if(a.data.results.length)l(a.data);else return}).catch(a=>console.log(a))):(y(),v(),m().then(({data:a})=>{l(a)}).catch(a=>console.log(a)),c.reset())}function U(t){t.preventDefault(),t.target.value==="Show all"&&(m().then(({data:e})=>{l(e)}).catch(e=>console.log(e)),c.reset()),o.category=t.target.value.replaceAll(" ","_").replaceAll("&","%26"),d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else if(!Array.isArray(e.data.results)||!e.data.results.length){u();return}}).catch(e=>console.log(e))}function G(t){if(t.preventDefault(),t.target.value==="Show_all"&&(y(),v(),m().then(({data:e})=>{l(e)}).catch(e=>console.log(e)),c.reset()),t.target.value==="A_to_Z"||t.target.value==="Z_to_A"){t.target.value==="A_to_Z"?o.byABC=!0:o.byABC=!1,o.byPrice=null,o.byPopularity=null,d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Cheap"||t.target.value==="Expensive"){t.target.value==="Cheap"?o.byPrice=!0:o.byPrice=!1,o.byABC=null,o.byPopularity=null,d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Popular"||t.target.value==="Not_popular"){t.target.value==="Not_popular"?o.byPopularity=!0:o.byPopularity=!1,o.byABC=null,o.byPrice=null,d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Show_all"){y();return}}const J=document.querySelector(".product-modal-content"),k=document.querySelector("body");function K(t){const e={openModalBtn:t,closeModalBtn:document.querySelector(".product-modal-close"),modal:document.querySelector(".product-backdrop")};e.openModalBtn.addEventListener("click",a);function a(n){n.target.tagName!=="BUTTON"&&(k.style.overflow="hidden",e.modal.classList.remove("is-hidden"),R(n.currentTarget),window.addEventListener("keydown",s),e.closeModalBtn.addEventListener("click",s))}function s(n){(n.key==="Escape"||n.currentTarget===e.closeModalBtn)&&(k.style.overflow="visible",J.innerHTML=`
      <div class="product-modal-content">Loading...</div>
      `,e.modal.classList.add("is-hidden"),window.removeEventListener("keydown",s),e.closeModalBtn.removeEventListener("click",s))}}function R(t){A(t.id).then(({data:e})=>{const a=document.querySelector(".product-modal-content");a.innerHTML=V(e)}).catch(e=>console.log(e))}function V(t){return`
  ...
  `}const j=document.querySelector(".product-list"),L=document.querySelector(".pagination");function l(t){L.classList.remove("visually-hidden"),j.innerHTML=W(t.results),document.querySelectorAll(".product-modal-open").forEach(a=>{a.addEventListener("click",K(a))})}function W(t){return t.map(({_id:e,name:a,img:s,category:n,price:p,size:g,is10PercentOff:h,popularity:B})=>`
        ...
        `).join("")}function X(t){t.preventDefault(),m().then(({data:e})=>{l(e)}).catch(e=>console.log(e)),c.reset(),y(),v()}function u(){const t=`
  ...
  `;j.innerHTML=t,document.querySelector(".js-link").addEventListener("click",X),L.classList.add("visually-hidden")}const Y=document.getElementById("tui-pagination-container");new S(Y,{});C().then(({data:t})=>{F(t),f()}).catch(t=>console.log(t));m().then(({data:t,data:{perPage:e,totalPages:a}})=>{l(t),f();const s=document.getElementById("tui-pagination-container");new S(s,{totalItems:e*a,itemsPerPage:e,visiblePages:5,centerAlign:!0,template:{page:'<a href="#" class="tui-page-btn">{{page}}</a>',currentPage:'<strong class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}}).on("beforeMove",async p=>{const g=p.page;try{const{data:h}=await r({page:g,limitPerPage:e});l(h)}catch(h){console.log(h)}})});q().then(({data:t})=>{x(t),f()}).catch(t=>console.log(t));E().then(({data:t})=>{N(t.toString().replaceAll("_"," ").split(",")),f()}).catch(t=>console.log(t));document.getElementById("loader-container");document.querySelector(".main");const tt=document.querySelector(".js-main-container");tt.addEventListener("click",t=>{if(t.target.classList.contains("js-buy-btn")){let e=t.target.closest(".js-product-card");T(e.dataset.id),f(),document.querySelector(".js-cart-numbers").textContent=P().products.length}});function et(t){const e=document.querySelectorAll(`.js-product-card[data-id="${t}"]`);e.length&&e.forEach(a=>{const s=a.querySelector(".js-buy-btn");s.style.backgroundColor="#6D8434",s.querySelector(".js-btn-first-ico").style.display="none",s.querySelector(".js-btn-second-ico").style.display="block",s.disabled=!0})}function f(){P().products.forEach(t=>{et(t.id)})}
