import{s as P,a as d,b as c,c as L,d as A,e as k}from"./assets/sprite-a6ecf29b.js";import"./assets/vendor-f9e4276b.js";const C=document.querySelector(".popular-product-list");function w(t){C.insertAdjacentHTML("beforeend",T(t))}function T(t){return t.map(({_id:e,name:a,img:l,category:n,size:g,popularity:f})=>`
    ...
    `).splice(0,2).join("")}let M={email:"",products:[]};function p(){const t={keyword:null,category:null,byABC:!0,byPrice:null,byPopularity:null,page:1,limit:6};return localStorage.setItem("filterParams",JSON.stringify(t)),s()}function i(t){localStorage.setItem("filterParams",JSON.stringify(t))}JSON.parse(localStorage.getItem("shoppingCartItems"))||localStorage.setItem("shoppingCartItems",JSON.stringify(M));function N(){return JSON.parse(localStorage.getItem("shoppingCartItems")).products.length}function s(){return JSON.parse(localStorage.getItem("filterParams"))}const I=document.querySelector(".cart-number");I.insertAdjacentHTML("beforeend",N());const u=document.querySelector("#filterForm");u.addEventListener("submit",O);const B=document.querySelector("#categories");u.elements.filterCategories.addEventListener("change",E);u.elements.filterMethod.addEventListener("change",F);let o;function m(){o=s()?s():p()}m();function D(t){B.insertAdjacentHTML("afterbegin",x(t))}function x(t){const e=t.map(a=>`
    <option value="${a}" class="js-option">${a}</option>
    `);return e.unshift('<option value="" disabled selected>Categories</option>'),e.join("")}function O(t){t.preventDefault();const{filterInput:e,filterCategories:a}=t.target.elements;z(e,a)}function z(t,e){t.value.trim().length?t.value.trim()?(o.keyword=t.value.trim(),e.value!==""&&(o.category=e.value),i(o),c(s()).then(a=>{if(console.log(a),a.data.results.length)r(a.data);else if(!Array.isArray(a.data.results)||!a.data.results.length){b();return}}).catch(a=>console.log(a))):e.value&&(o.category=e.value.trim(),i(o),c(s()).then(a=>{if(a.data.results.length)r(a.data);else return}).catch(a=>console.log(a))):(p(),m(),d().then(({data:a})=>{r(a)}).catch(a=>console.log(a)),u.reset())}function E(t){t.preventDefault(),t.target.value==="Show all"&&(d().then(({data:e})=>{r(e)}).catch(e=>console.log(e)),u.reset()),o.category=t.target.value.replaceAll(" ","_"),i(o),c(s()).then(e=>{if(e.data.results.length)r(e.data);else if(!Array.isArray(e.data.results)||!e.data.results.length){b();return}}).catch(e=>console.log(e))}function F(t){if(t.preventDefault(),t.target.value==="A_to_Z"||t.target.value==="Z_to_A"){t.target.value==="A_to_Z"?o.byABC=!0:o.byABC=!1,i(o),c(s()).then(e=>{if(e.data.results.length)r(e.data);else return}).catch(e=>console.log(e));return}else if(t.target.value==="Cheap"||t.target.value==="Expensive"){t.target.value==="Cheap"?o.byPrice=!0:o.byPrice=!1,i(o),c(s()).then(e=>{if(e.data.results.length)r(e.data);else return}).catch(e=>console.log(e));return}else if(t.target.value==="Popular"||t.target.value==="Not_popular"){t.target.value==="Not_popular"?o.byPopularity=!0:o.byPopularity=!1,i(o),c(s()).then(e=>{if(e.data.results.length)r(e.data);else return}).catch(e=>console.log(e));return}else if(t.target.value==="Show_all"){p();return}}const v=document.querySelector(".product-list");function r(t){v.innerHTML=J(t.results)}function J(t){return t.map(({_id:e,name:a,img:l,category:n,price:g,size:f,is10PercentOff:S,popularity:$})=>`
