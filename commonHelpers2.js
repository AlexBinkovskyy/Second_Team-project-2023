import{s as A,d as y,e as f,f as d,h as r,i,j as B,k as C,l as q,m as E,n as T,g as S}from"./assets/workWithCart-5e9a4738.js";import{P as k}from"./assets/vendor-8aa786c2.js";const w="/Second_Team-project-2023/assets/discount-1f26b8b0.svg",b="/Second_Team-project-2023/assets/check-ico-267a6277.svg",_=document.querySelector(".popular-product-list");async function x(t){_.insertAdjacentHTML("beforeend",D(t))}function D(t){return t.map(({_id:e,name:a,img:s,category:n,size:p,popularity:g})=>`
    ...
    `);return e.unshift('<option value="" disabled selected>Categories</option>'),e.join("")}function Z(t){t.preventDefault();const{filterInput:e,filterCategories:a}=t.target.elements;Q(e,a)}function Q(t,e){t.value.trim().length?t.value.trim()?(o.keyword=t.value.trim(),e.value!==""&&(o.category=e.value),d(o),r(i()).then(a=>{if(console.log(a),a.data.results.length)l(a.data);else if(!Array.isArray(a.data.results)||!a.data.results.length){u();return}}).catch(a=>console.log(a))):e.value&&(o.category=e.value.trim(),d(o),r(i()).then(a=>{if(a.data.results.length)l(a.data);else return}).catch(a=>console.log(a))):(y(),v(),f().then(({data:a})=>{l(a)}).catch(a=>console.log(a)),c.reset())}function U(t){t.preventDefault(),t.target.value==="Show all"&&(f().then(({data:e})=>{l(e)}).catch(e=>console.log(e)),c.reset()),o.category=t.target.value.replaceAll(" ","_").replaceAll("&","%26"),d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else if(!Array.isArray(e.data.results)||!e.data.results.length){u();return}}).catch(e=>console.log(e))}function G(t){if(t.preventDefault(),t.target.value==="Show_all"&&(y(),v(),f().then(({data:e})=>{l(e)}).catch(e=>console.log(e)),c.reset()),t.target.value==="A_to_Z"||t.target.value==="Z_to_A"){t.target.value==="A_to_Z"?o.byABC=!0:o.byABC=!1,o.byPrice=null,o.byPopularity=null,d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Cheap"||t.target.value==="Expensive"){t.target.value==="Cheap"?o.byPrice=!0:o.byPrice=!1,o.byABC=null,o.byPopularity=null,d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Popular"||t.target.value==="Not_popular"){t.target.value==="Not_popular"?o.byPopularity=!0:o.byPopularity=!1,o.byABC=null,o.byPrice=null,d(o),r(i()).then(e=>{if(e.data.results.length)l(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Show_all"){y();return}}const J=document.querySelector(".product-modal-content"),P=document.querySelector("body");function K(t){const e={openModalBtn:t,closeModalBtn:document.querySelector(".product-modal-close"),modal:document.querySelector(".product-backdrop")};e.openModalBtn.addEventListener("click",a);function a(n){n.target.tagName!=="BUTTON"&&(P.style.overflow="hidden",e.modal.classList.remove("is-hidden"),R(n.currentTarget),window.addEventListener("keydown",s),e.closeModalBtn.addEventListener("click",s))}function s(n){(n.key==="Escape"||n.currentTarget===e.closeModalBtn)&&(P.style.overflow="visible",J.innerHTML=`
  ...
  `}const j=document.querySelector(".product-list"),L=document.querySelector(".pagination");function l(t){L.classList.remove("visually-hidden"),j.innerHTML=W(t.results),document.querySelectorAll(".product-modal-open").forEach(a=>{a.addEventListener("click",K(a))})}function W(t){return t.map(({_id:e,name:a,img:s,category:n,price:p,size:g,is10PercentOff:h,popularity:M})=>`
        ...
        `).join("")}function X(t){t.preventDefault(),f().then(({data:e})=>{l(e)}).catch(e=>console.log(e)),c.reset(),y(),v()}function u(){const t=`
  ...
  `;j.innerHTML=t,document.querySelector(".js-link").addEventListener("click",X),L.classList.add("visually-hidden")}const Y=document.getElementById("tui-pagination-container");new k(Y,{});C().then(({data:t})=>{F(t),m()}).catch(t=>console.log(t));f().then(({data:t,data:{perPage:e,totalPages:a}})=>{l(t),m();const s=document.getElementById("tui-pagination-container");new k(s,{totalItems:e*a,itemsPerPage:e,visiblePages:5,centerAlign:!0,template:{page:'<a href="#" class="tui-page-btn">{{page}}</a>',currentPage:'<strong class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}}).on("beforeMove",async p=>{const g=p.page;try{const{data:h}=await r({page:g,limitPerPage:e});l(h)}catch(h){console.log(h)}})});q().then(({data:t})=>{x(t),m()}).catch(t=>console.log(t));E().then(({data:t})=>{N(t.toString().replaceAll("_"," ").split(",")),m()}).catch(t=>console.log(t));document.getElementById("loader-container");document.querySelector(".main");const tt=document.querySelector(".js-main-container");tt.addEventListener("click",t=>{if(t.target.classList.contains("js-buy-btn")){let e=t.target.closest(".js-product-card");T(e.dataset.id),m(),document.querySelector(".js-cart-numbers").textContent=S().products.length}});function et(t){const e=document.querySelectorAll(`.js-product-card[data-id="${t}"]`);e.length&&e.forEach(a=>{const s=a.querySelector(".js-buy-btn");s.style.backgroundColor="#6D8434",s.querySelector(".js-btn-first-ico").style.display="none",s.querySelector(".js-btn-second-ico").style.display="block",s.disabled=!0})}function m(){S().products.forEach(t=>{et(t.id)})}
