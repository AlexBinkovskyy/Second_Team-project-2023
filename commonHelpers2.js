import{s as M,a as d,b as c,c as b,d as S,e as $}from"./assets/sprite-163abfe1.js";import"./assets/vendor-a61d8330.js";const f="/Second_Team-project-2023/assets/discount-1f26b8b0.svg",q=document.querySelector(".popular-product-list");function C(t){q.insertAdjacentHTML("beforeend",T(t))}function T(t){return t.map(({_id:e,name:o,img:n,category:r,size:g,popularity:m})=>`
    ...
    `);return e.unshift('<option value="" disabled selected>Categories</option>'),e.join("")}function F(t){t.preventDefault();const{filterInput:e,filterCategories:o}=t.target.elements;z(e,o)}function z(t,e){t.value.trim().length?t.value.trim()?(a.keyword=t.value.trim(),e.value!==""&&(a.category=e.value),i(a),c(l()).then(o=>{if(console.log(o),o.data.results.length)s(o.data);else if(!Array.isArray(o.data.results)||!o.data.results.length){w();return}}).catch(o=>console.log(o))):e.value&&(a.category=e.value.trim(),i(a),c(l()).then(o=>{if(o.data.results.length)s(o.data);else return}).catch(o=>console.log(o))):(p(),y(),d().then(({data:o})=>{s(o)}).catch(o=>console.log(o)),u.reset())}function J(t){t.preventDefault(),t.target.value==="Show all"&&(d().then(({data:e})=>{s(e)}).catch(e=>console.log(e)),u.reset()),a.category=t.target.value.replaceAll(" ","_").replaceAll("&","%26"),i(a),c(l()).then(e=>{if(e.data.results.length)s(e.data);else if(!Array.isArray(e.data.results)||!e.data.results.length){w();return}}).catch(e=>console.log(e))}function H(t){if(t.preventDefault(),t.target.value==="A_to_Z"||t.target.value==="Z_to_A"){t.target.value==="A_to_Z"?a.byABC=!0:a.byABC=!1,i(a),c(l()).then(e=>{if(e.data.results.length)s(e.data);else return}).catch(e=>console.log(e));return}else if(t.target.value==="Cheap"||t.target.value==="Expensive"){t.target.value==="Cheap"?a.byPrice=!0:a.byPrice=!1,i(a),c(l()).then(e=>{if(e.data.results.length)s(e.data);else return}).catch(e=>console.log(e));return}else if(t.target.value==="Popular"||t.target.value==="Not_popular"){t.target.value==="Not_popular"?a.byPopularity=!0:a.byPopularity=!1,i(a),c(l()).then(e=>{if(e.data.results.length)s(e.data);else return}).catch(e=>console.log(e));return}else if(t.target.value==="Show_all"){p();return}}function Z(t){const e={openModalBtn:t,closeModalBtn:document.querySelector(".product-modal-close"),modal:document.querySelector(".product-backdrop")};e.openModalBtn.addEventListener("click",o),e.closeModalBtn.addEventListener("click",o);function o(){e.modal.classList.toggle("is-hidden")}}const L=document.querySelector(".product-list");document.querySelector(".product-modal-window");function s(t){L.innerHTML=Q(t.results);const e=document.querySelector(".product-modal-open");console.log(e),e.addEventListener("click",Z(e))}function Q(t){return t.map(({_id:e,name:o,img:n,category:r,price:g,size:m,is10PercentOff:k,popularity:A})=>`
