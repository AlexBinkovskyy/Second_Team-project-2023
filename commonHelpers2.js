import{s as M,d,e as i,f as g,h as m,i as u,j as B,k as C,l as q,m as E,n as _,g as k}from"./assets/workWithCart-b634ace8.js";import{P as w}from"./assets/vendor-8aa786c2.js";const j="/Second_Team-project-2023/assets/discount-1f26b8b0.svg",b="/Second_Team-project-2023/assets/check-ico-267a6277.svg",T=document.querySelector(".popular-product-list");async function x(t){T.insertAdjacentHTML("beforeend",D(t))}function D(t){return t.map(({_id:e,name:a,img:s,category:l,size:f,popularity:r})=>`
    ...
    `).splice(0,2).join("")}const c=document.querySelector("#filterForm");c.addEventListener("submit",Z);const H=document.querySelector("#single");c.elements.filterCategories.addEventListener("change",U);c.elements.filterMethod.addEventListener("change",G);let o;function v(){i()?(o=i(),o.keyword!==null&&(c.filterInput.value=o.keyword)):g()}v();d(i()).then(({data:t})=>{if(t.results.length)console.log("0",t),n(t),h();else if(!Array.isArray(t.results)||!t.results.length){p();return}});function N(t){if(H.innerHTML=O(t),o.category!==null){const e=c.filterCategories.options;for(const a of e)if(a.value.replaceAll(" ","_").replaceAll("&","%26")===o.category){a.selected=!0;break}}}function O(t){const e=t.map(a=>`
    <option value="${a}" class="js-option">${a}</option>
    `);return e.unshift('<option value="" class="js-option" disabled selected>Categories</option>'),e.push('<option value="Show all" class="js-option">Show all</option>'),e.join("")}function Z(t){t.preventDefault();const{filterInput:e,filterCategories:a}=t.target.elements;Q(e,a)}function Q(t,e){t.value.trim().length?t.value.trim()?(o.keyword=t.value.trim(),e.value!==""&&(o.category=e.value.replaceAll(" ","_").replaceAll("&","%26")),u(o),d(i()).then(a=>{if(console.log(a),a.data.results.length)n(a.data);else if(!Array.isArray(a.data.results)||!a.data.results.length){p();return}}).catch(a=>console.log(a))):e.value&&(o.category=e.value.trim(),u(o),d(i()).then(a=>{if(a.data.results.length)n(a.data);else return}).catch(a=>console.log(a))):(g(),v(),m().then(({data:a})=>{n(a)}).catch(a=>console.log(a)),c.reset())}function U(t){t.preventDefault(),t.target.value==="Show all"&&(g(),c.reset(),m().then(({data:e})=>{n(e)}).catch(e=>console.log(e)),c.reset()),o.category=t.target.value.replaceAll(" ","_").replaceAll("&","%26"),u(o),d(i()).then(e=>{if(e.data.results.length)n(e.data);else if(!Array.isArray(e.data.results)||!e.data.results.length){p();return}}).catch(e=>console.log(e))}function G(t){if(t.preventDefault(),t.target.value==="Show_all"&&(g(),v(),m().then(({data:e})=>{n(e)}).catch(e=>console.log(e)),c.reset()),t.target.value==="A_to_Z"||t.target.value==="Z_to_A"){t.target.value==="A_to_Z"?o.byABC=!0:o.byABC=!1,o.byPrice=null,o.byPopularity=null,u(o),d(i()).then(e=>{if(e.data.results.length)n(e.data);else{p();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Cheap"||t.target.value==="Expensive"){t.target.value==="Cheap"?o.byPrice=!0:o.byPrice=!1,o.byABC=null,o.byPopularity=null,u(o),d(i()).then(e=>{if(e.data.results.length)n(e.data);else{p();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Popular"||t.target.value==="Not_popular"){t.target.value==="Not_popular"?o.byPopularity=!0:o.byPopularity=!1,o.byABC=null,o.byPrice=null,u(o),d(i()).then(e=>{if(e.data.results.length)n(e.data);else{p();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Show_all"){g();return}}const J=document.querySelector(".product-modal-content"),S=document.querySelector("body");function K(t){const e={openModalBtn:t,closeModalBtn:document.querySelector(".product-modal-close"),modal:document.querySelector(".product-backdrop")};e.openModalBtn.addEventListener("click",a);function a(l){l.target.tagName!=="BUTTON"&&(S.style.overflow="hidden",e.modal.classList.remove("is-hidden"),R(l.currentTarget),window.addEventListener("keydown",s),e.closeModalBtn.addEventListener("click",s))}function s(l){(l.key==="Escape"||l.currentTarget===e.closeModalBtn)&&(S.style.overflow="visible",J.innerHTML=`
  ...
  `}const W=document.getElementById("tui-pagination-container");new w(W,{});const A=document.querySelector(".product-list"),L=document.querySelector(".pagination");function n({results:t,perPage:e,totalPages:a}){L.classList.remove("visually-hidden"),console.log("1",t),A.innerHTML=X(t),document.querySelectorAll(".product-modal-open").forEach(r=>{r.addEventListener("click",K(r))});const l=document.getElementById("tui-pagination-container");new w(l,{totalItems:e*a,itemsPerPage:e,visiblePages:5,centerAlign:!0,template:{page:'<a href="#" class="tui-page-btn">{{page}}</a>',currentPage:'<strong class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}}).on("beforeMove",async r=>{const $=r.page;try{const{data:y}=await d({page:$,limitPerPage:e});n(y),h()}catch(y){console.log(y)}})}function X(t){return t.map(({_id:e,name:a,img:s,category:l,price:f,size:r,is10PercentOff:$,popularity:y})=>`
        ...
        `).join("")}function Y(t){t.preventDefault(),m().then(({data:e})=>{n(e)}).catch(e=>console.log(e)),c.reset(),g(),v()}function p(){const t=`
