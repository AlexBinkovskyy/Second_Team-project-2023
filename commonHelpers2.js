import{s as k,d as y,e as m,f as d,h as i,i as c,j,k as B,l as A,m as E,n as q}from"./assets/workWithCart-977308f3.js";import{P}from"./assets/vendor-8aa786c2.js";const S="/Second_Team-project-2023/assets/discount-1f26b8b0.svg",f="/Second_Team-project-2023/assets/check-ico-267a6277.svg",C=document.querySelector(".popular-product-list");async function T(t){C.insertAdjacentHTML("beforeend",_(t));const e=document.querySelector(".click-buy");e.addEventListener("click",()=>{e.style.display="none",a.style.display="block"});const a=document.querySelector(".popular-check")}function _(t){return t.map(({_id:e,name:a,img:l,category:s,size:p,popularity:g})=>`
    ...
    `).splice(0,2).join("")}const r=document.querySelector("#filterForm");r.addEventListener("submit",N);const F=document.querySelector("#single");r.elements.filterCategories.addEventListener("change",Z);r.elements.filterMethod.addEventListener("change",Q);let o;async function b(){c()?(o=c(),o.keyword!==null?(r.filterInput.value=o.keyword,await i(c()).then(t=>{if(console.log(t),t.data.results.length)n(t.data);else if(!Array.isArray(t.data.results)||!t.data.results.length){u();return}})):o.category):y()}b();function H(t){F.insertAdjacentHTML("afterbegin",I(t))}function I(t){const e=t.map(a=>`
    <option value="${a}" class="js-option">${a}</option>
    `);return e.unshift('<option value="" disabled selected>Categories</option>'),e.join("")}function N(t){t.preventDefault();const{filterInput:e,filterCategories:a}=t.target.elements;O(e,a)}function O(t,e){t.value.trim().length?t.value.trim()?(o.keyword=t.value.trim(),e.value!==""&&(o.category=e.value),d(o),i(c()).then(a=>{if(console.log(a),a.data.results.length)n(a.data);else if(!Array.isArray(a.data.results)||!a.data.results.length){u();return}}).catch(a=>console.log(a))):e.value&&(o.category=e.value.trim(),d(o),i(c()).then(a=>{if(a.data.results.length)n(a.data);else return}).catch(a=>console.log(a))):(y(),b(),m().then(({data:a})=>{n(a)}).catch(a=>console.log(a)),r.reset())}function Z(t){t.preventDefault(),t.target.value==="Show all"&&(m().then(({data:e})=>{n(e)}).catch(e=>console.log(e)),r.reset()),o.category=t.target.value.replaceAll(" ","_").replaceAll("&","%26"),d(o),i(c()).then(e=>{if(e.data.results.length)n(e.data);else if(!Array.isArray(e.data.results)||!e.data.results.length){u();return}}).catch(e=>console.log(e))}function Q(t){if(t.preventDefault(),t.target.value==="Show_all"&&(y(),b(),m().then(({data:e})=>{n(e)}).catch(e=>console.log(e)),r.reset()),t.target.value==="A_to_Z"||t.target.value==="Z_to_A"){t.target.value==="A_to_Z"?o.byABC=!0:o.byABC=!1,o.byPrice=null,o.byPopularity=null,d(o),i(c()).then(e=>{if(e.data.results.length)n(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Cheap"||t.target.value==="Expensive"){t.target.value==="Cheap"?o.byPrice=!0:o.byPrice=!1,o.byABC=null,o.byPopularity=null,d(o),i(c()).then(e=>{if(e.data.results.length)n(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Popular"||t.target.value==="Not_popular"){t.target.value==="Not_popular"?o.byPopularity=!0:o.byPopularity=!1,o.byABC=null,o.byPrice=null,d(o),i(c()).then(e=>{if(e.data.results.length)n(e.data);else{u();return}}).catch(e=>console.log(e));return}else if(t.target.value==="Show_all"){y();return}}const U=document.querySelector(".product-modal-content"),$=document.querySelector("body");function G(t){const e={openModalBtn:t,closeModalBtn:document.querySelector(".product-modal-close"),modal:document.querySelector(".product-backdrop")};e.openModalBtn.addEventListener("click",a);function a(s){s.target.tagName!=="BUTTON"&&($.style.overflow="hidden",e.modal.classList.remove("is-hidden"),J(s.currentTarget),window.addEventListener("keydown",l),e.closeModalBtn.addEventListener("click",l))}function l(s){(s.key==="Escape"||s.currentTarget===e.closeModalBtn)&&($.style.overflow="visible",U.innerHTML=`
  ...
  `}const w=document.querySelector(".product-list"),L=document.querySelector(".pagination");function n(t){L.classList.remove("visually-hidden"),w.innerHTML=R(t.results),document.querySelectorAll(".product-modal-open").forEach(a=>{a.addEventListener("click",G(a))})}function R(t){return t.map(({_id:e,name:a,img:l,category:s,price:p,size:g,is10PercentOff:h,popularity:M})=>`
        ...
        `).join("")}function V(t){t.preventDefault(),m().then(({data:e})=>{n(e)}).catch(e=>console.log(e)),r.reset(),y(),b()}function u(){const t=`
  ...
  `;w.innerHTML=t,document.querySelector(".js-link").addEventListener("click",V),L.classList.add("visually-hidden")}const W=document.getElementById("tui-pagination-container");new P(W,{});B().then(({data:t})=>{D(t)}).catch(t=>console.log(t));m().then(({data:t,data:{perPage:e,totalPages:a}})=>{n(t);const l=document.getElementById("tui-pagination-container");new P(l,{totalItems:e*a,itemsPerPage:e,visiblePages:5,centerAlign:!0,template:{page:'<a href="#" class="tui-page-btn">{{page}}</a>',currentPage:'<strong class="tui-page-btn tui-is-selected">{{page}}</strong>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}}"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}}).on("beforeMove",async p=>{const g=p.page;try{const{data:h}=await i({page:g,limitPerPage:e});n(h)}catch(h){console.log(h)}})});A().then(({data:t})=>{T(t)}).catch(t=>console.log(t));E().then(({data:t})=>{H(t.toString().replaceAll("_"," ").split(","))}).catch(t=>console.log(t));document.getElementById("loader-container");document.querySelector(".main");const X=document.querySelector(".js-main-container");X.addEventListener("click",t=>{if(t.target.classList.contains("js-buy-btn")){let e=t.target.closest(".js-product-card");q(e.dataset.id),Y(e.dataset.id)}});function Y(t){const e=document.querySelectorAll(`.js-product-card[data-id="${t}"]`),a="";console.log(e),e.forEach(l=>{const s=l.querySelector(".js-buy-btn");s.innerHTML=`<svg class="img-icon" width="12" height="12" style="stroke:"#fff""><use href="${k}#${a}"></use></svg>`,s.disabled=!0})}
