import{a as l}from"./vendor-a61d8330.js";(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))n(e);new MutationObserver(e=>{for(const s of e)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function i(e){const s={};return e.integrity&&(s.integrity=e.integrity),e.referrerpolicy&&(s.referrerPolicy=e.referrerpolicy),e.crossorigin==="use-credentials"?s.credentials="include":e.crossorigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(e){if(e.ep)return;e.ep=!0;const s=i(e);fetch(e.href,s)}})();const m="https://food-boutique.b.goit.study/api/products/",h="categories",y="popular",C="discount",c={MAIN_URL:m,categories:h,popular:y,discount:C};let $=1,a=6;async function b(){return window.innerWidth>=1440?a=9:window.innerWidth>=768?a=8:a=6,await l(`${c.MAIN_URL}?page=${$}&limit=${a}`)}async function v(){return await l(`${c.MAIN_URL}/${c.popular}?limit=${a}`)}async function S(){return await l(`${c.MAIN_URL}/${c.discount}`)}async function P(){return await l(`${c.MAIN_URL}/${c.categories}`)}const L=document.querySelector(".popular-product");function w(t){L.insertAdjacentHTML("beforeend",I(t))}function I(t){return t.map(({_id:o,name:i,img:n,category:e,size:s,popularity:r})=>`
        ...
        <li class="discount-product">
         ...
         <img class="discount-svg" src="${M}" alt="discount-icon" width="60" height="60"/>
            ...
            <div class="discount-text-item">
                ...
                <img class="discount-buy-svg" src="${N}" alt="shop-icon" width="18" height="18"/>
  </svg>
</button>
</div> 
            </div>
    ...
    `).join("")}const T=document.querySelector(".product-list");function A(t){T.insertAdjacentHTML("beforeend",q(t.results))}function q(t){return t.map(({_id:o,name:i,img:n,category:e,price:s,size:r,is10PercentOff:R,popularity:g})=>`
      ...
      `).join("")}let x={email:"",products:[{productId:"",amount:1}]};function p(){const t={keyword:null,category:null,page:1,limit:6};return localStorage.setItem("filterParams",JSON.stringify(t)),d()}function _(t){localStorage.setItem("filterParams",JSON.stringify(t))}JSON.parse(localStorage.getItem("shoppingCartItems"))||localStorage.setItem("shoppingCartItems",JSON.stringify(x));function d(){return JSON.parse(localStorage.getItem("filterParams"))}const k=document.querySelector("#filterForm");k.addEventListener("submit",f);const B=document.querySelector("#categories"),F=document.querySelector(".js-option");F.addEventListener("change",f);let u=d()?d():p();function J(t){B.insertAdjacentHTML("afterbegin",D(t))}function D(t){const o=t.map(i=>`
    <option value="${i}" class="js-option">${i}</option>
    `);return o.unshift('<option value="" disabled selected>Categories</option>'),o.join("")}function f(t){t.preventDefault();const{filterInput:o,filterCategories:i,filterMethod:n}=t.target.elements;z(o)}function z(t){console.log(u),t.value.trim()?(u.keyword=`${t.value.trim()}`,_(u)):p()}S().then(({data:t})=>{O(t)}).catch(t=>console.log(t));b().then(({data:t})=>{A(t)}).catch(t=>console.log(t));v().then(({data:t})=>{w(t)}).catch(t=>console.log(t));P().then(({data:t})=>{J(t)}).catch(t=>console.log(t));
//# sourceMappingURL=main-e0f52651.js.map
